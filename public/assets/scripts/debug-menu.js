...
if (DEBUG_ENABLED) initDebugMenu();
...
async function initDebugMenu() {
  ...
  `;
    ...
    cachePanel.style.cssText = lsPanel.style.cssText;
    ...
    `;
        ...
        if (cacheNames.length === 0) {
            html += `
            <div style="padding:12px 14px; color:#555;">
                vacío
            </div>
        `;
        }
    ...
    lsPanel.addEventListener("click", (e) => {
        e.stopPropagation();
    });
    ...
    function renderLsPanel() {
        ...
        lsPanel.innerHTML = `
            ...
            ${keys.length === 0 ? '<div style="padding:12px 14px; color:#555;">vacío</div>' : ""}
            ${keys
                .map(
                    (k) => `
                <div style="display:flex; align-items:center; padding:6px 12px; gap:8px;
                            border-bottom:1px solid #1e1e30;">
                    <span style="flex:1; color:#ccc; font-size:12px; overflow:hidden;
                                 text-overflow:ellipsis; white-space:nowrap;">${k}</span>
                    <span style="font-size:10px; color:#555; max-width:60px; overflow:hidden;
    ...
    async function buildMenu() {
        ...
        menu.innerHTML = `
            ...
            <div class="dm-section">
                ...
                <div class="dm-row"><span class="key">Versión de SW</span><span class="val">${sw.version}</span></div>
            </div>
            ...
            <div class="dm-section">
                ...
                <div class="dm-row">
                    ...
                    <span class="val ${sw.active ? "ok" : "err"}">${sw.active ? "activo" : "inactivo"}</span>
                </div>
                <div class="dm-row" id="vc-cache-toggle">
                    <span class="key">Caché ›</span>
                    <span class="val">${cacheCount} archivos</span>
                </div>
            ...
            <div class="dm-section">
                ...
                <div class="dm-row" id="vc-clear-cache"><span class="key"><span class="material-symbols-rounded">delete</span>Limpiar caché</span></div>
                ...
                <div class="dm-row" id="vc-log-ls"><span class="key"><span class="material-symbols-rounded">terminal</span>Ver localStorage en consola</span></div>
            </div>
        `;
        ...
        document
            ...
            ?.addEventListener("click", async () => {
                ...
                alert(`Cache limpiado (${keys.length} stores)`);
                closeMenu();
            });
        ...
        document.getElementById("vc-log-ls")?.addEventListener("click", () => {
            console.table(
                Object.fromEntries(
        ...
        document
            ...
            ?.addEventListener("click", async (e) => {
                ...
                cachePanel.style.left = menuRect.right + 8 + "px";
                cachePanel.style.top = menuRect.top + "px";
        ...
        document
            ...
            ?.addEventListener("click", (e) => {
                ...
                renderLsPanel();
                const menuRect = menu.getBoundingClientRect();
                lsPanel.style.left = menuRect.right + 8 + "px";
                lsPanel.style.top = menuRect.top + "px";
